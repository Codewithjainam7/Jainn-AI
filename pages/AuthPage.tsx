...
export const AuthPage: React.FC<AuthPageProps> = ({ onLogin, onNavigate }) => {
  ...
  const handleAuth = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
        localStorage.setItem('jainnUser', JSON.stringify(mockUser));
        onLogin(mockUser);
      ...
      <div className="w-full lg:w-1/2 p-8 sm:p-12 lg:p-24 flex flex-col justify-center relative">
        ...
        <div className="max-w-md w-full mx-auto transition-opacity duration-300" style={{ opacity: isAnimating ? 0 : 1 }}>
            ...
            <div>
              ...
              {!isLogin && password && (
                  <div className="flex gap-1 mt-2 h-1">
                      {[1,2,3,4].map(i => (
                          <div key={i} className={`flex-1 rounded-full transition-colors ${i <= strength ? (strength > 2 ? 'bg-green-500' : 'bg-yellow-500') : 'bg-gray-200 dark:bg-gray-700'}`}></div>
                      ))}
                  </div>
              )}
            </div>
            ...
            {!isLogin && (
                ...
                <div className="relative">
                    ...
                    <input 
                        ...
                        className={`w-full pl-10 pr-4 py-3 rounded-xl bg-white dark:bg-[#161B22] border ${confirmPassword && password !== confirmPassword ? 'border-red-500 focus:ring-red-500' : 'border-gray-200 dark:border-white/10 focus:ring-blue-500'} outline-none transition-all dark:text-white`}
                        placeholder="••••••••"
                    />
                </div>
                </div>
          ...
          <div className="space-y-3">
            <button 
                type="button"
                className="w-full py-3 px-4 rounded-xl border border-gray-200 dark:border-white/10 flex items-center justify-center gap-3 hover:bg-gray-50 dark:hover:bg-white/5 transition-all text-gray-700 dark:text-gray-200 font-medium bg-white dark:bg-[#161B22]"
            >
            ...
            <button 
                type="button"
                onClick={handleGuest}
                className="w-full py-3 px-4 rounded-xl border border-dashed border-gray-300 dark:border-white/20 flex items-center justify-center gap-3 hover:bg-gray-50 dark:hover:bg-white/5 transition-all text-gray-500 hover:text-blue-500 font-medium"
            >
